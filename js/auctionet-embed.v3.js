...
(function() {
  ...
  (function(window) {
    "use strict";
    var ONE_DAY, ONE_HOUR, ONE_MINUTE, host, locales, scriptTag;
    scriptTag = "";
    host = "";
    ...
    return window.auctionet = {
      ...
      embed: function(options) {
        var data, document, renderedTemplate;
        ...
        return this.loadExternalItems(this.settings.initialFilter);
      },
      setLocale: function() {
        return this.text = locales[this.settings.locale];
      },
      setHeading: function() {
      ...
      drawItem: function(item) {
        ...
        endsInValue = this.settings.ended ? (date = new Date(item.ends_at * 1000), date.toLocaleDateString()) : this.timeDifference(new Date, new Date(item.ends_at * 1000));
        data = {
      ...
      apiUrl: function(filter) {
        var url;
        url = "https://auctionet.com/api/v2/items.json?locale=" + this.settings.locale + "&order=" + filter + "&per_page=" + this.settings.howManyItems;
        ...
        if (this.settings.categoryId) {
          url += "&category_id=" + this.settings.categoryId;
        }
        if (this.settings.eventId) {
          url += "&event_id=" + this.settings.eventId;
        }
        if (this.settings.ended) {
          url += "&ended=" + this.settings.ended;
        }
        return url;
      },
      linkUrl: function() {
        var url;
        url = "https://auctionet.com/" + this.settings.locale + "/search?";
        if (this.settings.companyId) {
          url += "company_id=" + this.settings.companyId;
        }
        if (this.settings.query) {
